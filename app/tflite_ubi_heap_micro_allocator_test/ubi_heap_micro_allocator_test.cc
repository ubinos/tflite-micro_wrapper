...
TF_LITE_MICRO_TEST(TestMockModelAllocationByNonPersistentMemoryPlannerShim) {
  ...
  tflite::AllOpsResolver op_resolver = tflite::testing::GetOpResolver();
  tflite::UbiHeapBufferAllocator* simple_allocator = tflite::UbiHeapBufferAllocator::Create();
  tflite::UbiHeapMicroAllocator* allocator = tflite::UbiHeapMicroAllocator::Create(simple_allocator, &planner);
  TF_LITE_MICRO_EXPECT(allocator != nullptr);
  tflite::SubgraphAllocations* subgraph_allocations =
      allocator->StartModelAllocation(model);
  TF_LITE_MICRO_EXPECT(nullptr != subgraph_allocations);
  ...
  uint8_t* aligned_arena = allocator->GetOverlayMemoryAddress();
...
TF_LITE_MICRO_TEST(TestMultiSubgraphNumScratchAllocations) {
  ...
  simple_allocator = tflite::UbiHeapBufferAllocator::Create();
  TF_LITE_MICRO_EXPECT(simple_allocator != nullptr);
  allocator = tflite::UbiHeapMicroAllocator::Create(simple_allocator);
  TF_LITE_MICRO_EXPECT(allocator != nullptr);
  ...
  size_t used_bytes_test;
  (void) used_bytes_test;
  used_bytes_test = allocator->used_bytes();
